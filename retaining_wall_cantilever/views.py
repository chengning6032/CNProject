...
def input_view(request):
    ...
    if request.method == 'POST':
        try:
            ...
            H_calc_total = H_fill_above_base + H_bp
            ...
            # 3.6 水壓力 (Water Pressure)
            # 水平分量: 0.5 * gamma_w * H^2
            # 垂直分量 (水重壓在斜面上): Ph * tan(theta)
            u_bot = gamma_w * h_sub
            ...
            M_total_driving = (P_soil_1_H * arm_soil_1) + \
                              (P_soil_2_H * arm_soil_2) + \
                              (force_water_H * arm_water) + \
                              M_sur_H + \
                              (P_seismic_H * arm_seismic) + \
                              M_dynamic_water
            ...
            # --- [4] 被動土壓力 (Passive Resistance) ---
            # Part A: Front Soil (作用於基版前緣 H_bp)
            ...
            if H_sk > 0:
                ...
                # 使用 Ratio Method 修正 M-O
                Kpe_base_MO = calculate_Mononobe_Okabe_Kpe(phi_base, delta_base, 0, 0, kh, kv)
                Kp_base_Coulomb = calculate_Coulomb_Kp(phi_base, delta_base, 0, 0)
                ratio = (Kpe_base_MO / Kp_base_Coulomb) if Kp_base_Coulomb > 0 else 1.0
                ...
                if H_water > 0:
                    # [修正] 之前這裡誤寫 gamma_base, 應為 gamma_sat_base
                    gamma_base_eff = gamma_sat_base - 1.0

                    # Static Pp2
                pp2_top = Kp_base * q_base_eff + 2 * c_base * math.sqrt(Kp_base)
                # [修正] 之前這裡誤用 gamma_base, 應為 gamma_base_eff
                ...
                # [修正] 同樣修正為 gamma_base_eff
                ppe2_bot = Kpe_base * (q_base_eff + gamma_base_eff * H_sk) * (1 - kv) + 2 * c_base * math.sqrt(Kpe_base)
                Ppe2_dynamic = 0.5 * (ppe2_top + ppe2_bot) * H_sk

                print("\n" + "*" * 60)
                print(f"{'止滑榫受力分析詳細 (SHEAR KEY DETAILS)':^60}")
                print("*" * 60)
                print(f"有效超載 q' = {q_base_eff:.3f} tf/m2 (Soil:{sigma_soil_front:.2f} + Conc:{sigma_concrete:.2f})")
                print(f"計算係數: Kp = {Kp_base:.4f}, Kpe(corr) = {Kpe_base:.4f}")
                print(f"靜態 Pp2 = {Pp2_static:.3f} tf/m (Top: {pp2_top:.2f}, Bot: {pp2_bot:.2f})")
                print("*" * 60 + "\n")
            ...
            FS_over = M_resist_total / M_total_driving if M_total_driving > 0 else 999.0
            ...
            X_bar = (M_resist_total - M_total_driving) / N_total if N_total > 0 else 0
            ...
            stability_results = {
                ...
                'M_resist': round(M_resist_total, 3), 'M_overturn': round(M_total_driving, 3),
                'F_resist': round(F_resist_total, 3), 'F_driving': round(P_total_driving_H, 3),
                'N_total': round(N_total, 3), 'Eccentricity': round(Eccentricity, 3),
            ...
            result_context = {
                ...
                'key_details': key_details, 'stability': stability_results,
    ...
    # Correct Indentation: This must be outside 'if POST'
